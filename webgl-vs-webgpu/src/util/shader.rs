...
pub async fn create_shader_module(
    ...
    insert_code_then_create(device, shader_name, None, label).await
}

#[allow(dead_code)]
pub async fn insert_code_then_create(
    device: &wgpu::Device,
    shader_name: &'static str,
    ...
    let code = request_shader_code(&base_dir, fold, &shader_name).await;
...
#[cfg(target_arch = "wasm32")]
async fn request_shader_code(base_dir: &str, fold: &str, shader_name: &str) -> String {
    let url_str = base_dir.to_string() + fold + "/" + shader_name + ".wgsl";
