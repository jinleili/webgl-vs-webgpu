...
wgpu (dawn 也是) 实现的 WebGPU 接口支持通过编译参数设置运行时要使用的图形接口后端，但在使用 WebGL 时功能特性会受到限制。这与 WebGL 库对接 WebGPU 接口有所不同：因为 WebGPU 接口是基于管线的，由管线映可直接射回状态机。然而，基于状态机的接口映射到管线时，不仅功能受限，还会极大地降低管线的复用性，从而导致性能损耗。
